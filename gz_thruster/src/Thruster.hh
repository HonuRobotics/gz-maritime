...
namespace maritime
{
  ...
  /// "Guidance and Control of Ocean Vehicles" in page 246. This plugin has
  /// three modes of operation. In the default mode it takes in a
  ...
  /// force to be applied using the said equation, or a normalized command in
  /// [-1, 1] which is scaled onto the thrust limits. In the default mode the
  ...
  /// `/model/{ns}/joint/{joint_name}/force`. Normalized mode publishes angular
  /// velocity, as the default mode does.
  ///
  /// ## System Parameters
  ///
  ...
  ///   `/model/{namespace}/joint/{joint_name}/cmd_vel` or
  ///   `/model/{namespace}/joint/{joint_name}/cmd_normalized` depending on the
  ///   mode of operation. If {topic} is set then the plugin will listen on
  ///   {namespace}/{topic}
  ///   [Optional]

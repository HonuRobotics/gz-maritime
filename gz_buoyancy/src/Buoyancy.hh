...
// does not collide with gz-sim's own Buoyancy when both libraries are on
// GZ_SIM_SYSTEM_PLUGIN_PATH. Still nested inside gz::sim, so every gz-sim
// type the body uses unqualified still resolves.
namespace maritime
{
  // Forward declaration
  ...
  /// asks to, so the world needs to know no vehicle's name in advance.
  /// [Type: bool]
  ///
  /// ## Services
  ///
  /// Both take a `gz.msgs.StringMsg` naming one entity and reply with a
  /// `gz.msgs.Boolean`. Names are scoped exactly as `<enable>` wants them:
  /// `model` or `model::link`, as the model was *spawned* rather than as its
  /// file names it. The reply reports that the request was queued, not that
  /// the entity exists; a name that matches nothing is not an error, because
  /// a vehicle may legitimately register before it is spawned.
  ///
  ...
  /// works in both modes, including against a name from `<enable>`.
  ///
  /// Both sets survive a world reset: a reset rewinds entities, not the
  /// decisions made about them. A model carrying the companion BuoyancyEnable
  /// plugin is the one exception, and in only one direction. That plugin does
  /// not implement Reset, so the reset destroys and reloads it, its Configure
  /// runs again and it re-asserts its own links — overriding a disable that
  /// was issued against it at runtime. Which is the right way round: the
  /// model's declaration is part of the state a reset returns to.
  ///
  /// The resulting modes:
  ///
  /// | `<enable>` | `<enable_by_default>` | Effect                          |
  /// |------------|-----------------------|---------------------------------|
  /// | absent     | absent (true)         | Everything floats. (Upstream.)  |
  /// | present    | absent (false)        | Only the listed. (Upstream.)    |
  /// | absent     | false                 | Nothing floats until a service  |
  /// |            |                       | call or a model-side plugin     |
  /// |            |                       | registers it.                   |

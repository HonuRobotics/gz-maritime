...
class gz::sim::maritime::BuoyancyEnablePrivate
{
    ...
    EntityComponentManager &_ecm)
{
  ...
  if (!stillPending.empty() && !this->dataPtr->warned)
  {
    ...
    gzwarn << "Buoyancy enable service [" << this->dataPtr->enableService
      ...
      << "succeeds, the world is not running the Buoyancy system from "
      << "gz_buoyancy." << std::endl;
  }
...
GZ_ADD_PLUGIN_ALIAS(BuoyancyEnable,
                          "gz::sim::maritime::BuoyancyEnable")

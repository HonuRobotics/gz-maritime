...
Decisions that need agreement are called out inline, where the reasoning for them sits.
...
> **Decision 1 — Scope within 1.4.** Does this task cover all of 1.4 except 1.4.1, and does 1.4.1 become its own issue once we confirm the competition server does not already exist?

I'd suggest the deliverable we can point to for this task is a working tutorial that launches a prototype VRX world with the BlueBoat, the BlueROV2 and the UAV all operating simultaneously. Worth noting that no such world exists today — gz-maritime ships one world, `open_water.sdf`, which is ocean-only with no ground and no landing surface. So "a prototype VRX world" is itself a piece of work as well. If that multi-domain world is out of scope, we should open an issue to track it for later.

> **Decision 2 — The deliverable.** Is the deliverable a tutorial running the BlueBoat, BlueROV2 and X500 simultaneously, and is building the prototype VRX world it needs inside this task or its own issue?

We should also decide if SITL is within the scope of this task. I'd suggest it be outside the scope for now because that work is still ongoing, and it limits, at least a little, the scope of this chunk of work. If we agree, we should put an issue up to integrate SITL for each vessel later. The practical consequence is that the tutorial drives all three vehicles over namespaced ROS topics. This suggests that the UAV should be spawned on land, because there is no controller to keep it airborne.
...
> **Decision 4 — Stock sensor suites.** Do we define one named "stock" sensor configuration per vehicle, and what goes in each?
...
> **Decision 5 — What 1.4.3 means here.** Is 1.4.3 satisfied by a repeatable measurement that normalizes for local compute, rather than by an optimization effort — and are RTF and FPS the two numbers we report? In-depth profiling then becomes its own issue.
...
> **Decision 6 — Which "multi-vehicle".** Do we build for case B, and attempt case A opportunistically — documented as a second example if it works, deferred to an issue if it does not, and blocking nothing either way?
...
> **Decision 7 — UUV depth in the prototype world.** Do we place the UUV at a fixed depth beyond 1.5 modal wavelengths, run a proximity test to characterize how bad the near-surface case is, and defer the fix to an issue rather than blocking on it?
...
> **Decision 8 — Branch point.** Do we wait for the normalized thruster interface to merge into `lyrical` before branching this work?
